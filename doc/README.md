...
| Feedback     | Envio de feedback por usuários logados e gerenciamento por super admin         | [feedback.md](./modules/feedback.md)         |
...
Rotas **sem** esse header: `GET /auth/me`, `GET /workspaces`, `POST /workspaces` (apenas `SUPER_ADMIN`), `GET /feedback/mine`, `GET /feedback` (apenas `SUPER_ADMIN`), `PATCH /feedback/:id` (apenas `SUPER_ADMIN`), `POST /auth/*` públicas, `GET /health`.
